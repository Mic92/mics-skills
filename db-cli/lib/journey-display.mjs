...
function displayJourneyLeg(leg) {
  const depTime = new Date(leg.departure).toLocaleTimeString("de-DE", {
    hour: "2-digit",
    minute: "2-digit",
  });
  const arrTime = new Date(leg.arrival).toLocaleTimeString("de-DE", {
    hour: "2-digit",
    minute: "2-digit",
  });
  ...
  if (leg.walking) {
    console.log(
      `  ${depTime} - ${arrTime}: Walk from ${leg.origin.name} to ${leg.destination.name}`,
    );
  } else if (leg.line) {
    const lineName =
      leg.line.name || `${leg.line.productName} ${leg.line.fahrtNr}`;
    console.log(
      `  ${depTime} - ${arrTime}: ${lineName} from ${leg.origin.name}${platform} to ${leg.destination.name}`,
    );

    // Show delays if any
    if (leg.departureDelay) {
      console.log(
        `    ⚠️  Departure delay: +${Math.round(leg.departureDelay / 60)} min`,
      );
    }
    if (leg.arrivalDelay) {
      console.log(
        `    ⚠️  Arrival delay: +${Math.round(leg.arrivalDelay / 60)} min`,
      );
    }
  }
}
...
export function displayJourney(journey, index) {
  ...
  console.log(
    `${startTime.toLocaleTimeString("de-DE", timeOptions)} → ${endTime.toLocaleTimeString("de-DE", timeOptions)} (${formatDuration(duration)})`,
  );
